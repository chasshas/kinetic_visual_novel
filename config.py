...
class Map:
    def __init__(self):
        self.map_data = None
        self.load_map_from_json("data/map_data.json")
        # TODO: 캐릭터 배치 등 초기화 로직
...
class Game:
    ...
    def create_title_screen_ui(self):
        ...
        new_game_button = pygame_gui.elements.UIButton(
            ...
            text='새 게임',
            manager=self.ui_manager,
            container=self.title_ui_panel,  # <--- 패널에 UI를 담습니다.
...
def run(self):
    while self.game_running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.USEREVENT:
                if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                    object_id = event.ui_element.get_object_ids()[0] if event.ui_element.get_object_ids() else None

                    if object_id == '#new_game_button':
                        self.start_new_game()
                    elif object_id == '#load_game_button':
                        self.load_game()
                    elif object_id == '#settings_button':
                        self.show_settings_popup()
                    elif object_id == '#credits_button':
                        self.show_credits_popup()
                    elif object_id == '#quit_button':
                        self.end_game()
                    elif object_id == '#close_settings_button':
                        if self.settings_popup:
                            self.settings_popup.kill()
                            self.settings_popup = None
                    elif object_id == '#close_credits_button':
                        if self.credits_popup:
                            self.credits_popup.kill()
                            self.credits_popup = None
        ...
        # 렌더링 파트
        screen.fill(BLACK)  # 화면을 검은색으로 클리어
...
if __name__ == "__main__":
    ...
    # 기본 테마 파일 생성 (선택사항)
    theme_path = 'data/themes/theme.json'
    if not os.path.exists(theme_path):
        ...
        try:
            ...
            print(f"Warning: Could not create theme file: {e}")
    ...
    if not os.path.exists(map_data_path):
        ...
        try:
            ...
            print(f"Warning: Could not create map data file: {e}")
